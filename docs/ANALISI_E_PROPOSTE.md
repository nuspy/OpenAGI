...
| M29 | Review incrociata con seconda AI (matrix granulare, consenso, override umano/AI) | P1 | utente (rev. 4) | REQUISITO — implementato (Fase 8) | Spec v1.2 §93, codice |
| M30 | Ground-check RAG nei guardrail (anti-allucinazione) | P1 | utente (rev. 4) | REQUISITO — implementato (Fase 8) | Spec v1.2 §94, codice |
| M31 | Nodi esogeni: direttive umane e fatti (imposti/opportunità) con peso, CRUD e rivalutazione | P0 | utente (rev. 5) | REQUISITO — implementato (Fase 9) | Spec v1.2 §95, codice |
| M32 | Interazione estesa: change-set nei thread, thread di scenario, consultazioni AI-initiated | P0 | utente (rev. 5) | REQUISITO — implementato (Fase 9) | Spec v1.2 §96, codice |
| M33 | Settings GUI strutturati (review matrix, grounding, esogeni) | P1 | utente (rev. 5) | REQUISITO — implementato (Fase 9) | Spec v1.2 §97, codice |
...
**Registro delle decisioni (30/08/2026)**: approvate M1–M17 e M19–M20; respinte M21 e M22 (la sezione SGI e le denominazioni restano come in v1.0); confermata l'attivazione asimmetrica dei guardrail Tier 2 (M23); aggiunto il requisito M28 (skills e MCP server importabili). **Rev. 4 (stessa data)**: aggiunti i requisiti M29 (review incrociata opzionale e granulare: per ogni tipo di decisione/analisi si configura attivazione, numero massimo di interazioni per il consenso e override in caso di mancato consenso — discussione con l'umano oppure decisione finale della AI principale che onora i consensi già concordati per quell'argomento) e M30 (ground-check RAG opzionale dentro i guardrail esistenti, su dati e decisioni, contro le allucinazioni). Entrambi implementati nella Fase 8 — vedi `docs/PHASE8.md`. **Rev. 5 (stessa data)**: aggiunti M31 (decisioni esterne e fatti come nodi di prima classe del grafo, con peso, integrazione a consenso, CRUD con rivalutazione delle relazioni), M32 (interazione estesa bidirezionale: le discussioni possono modificare scenario e target su richiesta umana; l'AI apre consultazioni su conflitti con le direttive e sync periodici) e M33 (gestione a pannelli dei settings di M29/M30/M31 in GUI) — implementati nella Fase 9, vedi `docs/PHASE9.md`. Le feature sono progettate come base della futura rete gerarchica/peer di istanze AI: studio in `docs/future_features.md`. **Conferma utente (30/08/2026)**: ratificati i default della Fase 9 — consenso d'integrazione sempre richiesto (`exogenous_require_consensus: true`, soglia di auto-tessitura a 0), eliminazione = ritiro event-sourced con cascata e orphan review, la conferma del thread d'integrazione vale come ratifica dei goal ivi proposti, impatti budget mai automatici. **Spec v1.2 (30/08/2026)**: M29–M33 formalizzate nel documento di specifica (nuove sezioni §93–§97, §65 aggiornato con gli event types post-1.1; numerazione esistente invariata). Su indicazione dell'utente corretto il nome dell'applicazione voce: **CallAPICall** (nei documenti era erroneamente "Call Happy Call") — corretto in spec, rationale, documenti operativi e codice; il paper .docx porta ancora il vecchio nome e sarà corretto alla prossima revisione tracked-changes.
